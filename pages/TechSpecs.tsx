...
import { Download, Cpu, Thermometer, Zap, Activity, BarChart3, TrendingUp, ShieldCheck, Cable, CheckCircle2, Box, ArrowRight, Layers, FileText } from 'lucide-react';
...
const StickySidebar = () => {
   ...
   useEffect(() => {
      const handleScroll = () => {
         const sections = ["visualizer", "reliability", "protocols", "blueprint", "specs"];
      ...
      // Attach listener to main scroll container if possible, or window if body scrolls
   ...
   return (
      <div className="flex flex-col gap-6">
         {[
            { id: "visualizer", label: "01. VISUALIZER" },
            { id: "reliability", label: "02. RELIABILITY" },
            { id: "protocols", label: "03. PROTOCOLS" },
            { id: "blueprint", label: "04. BLUEPRINT" },
            { id: "specs", label: "05. SPECS" },
         ].map((item) => (
            <button 
               key={item.id} 
            ...
            >
               <div className={`h-[2px] transition-all duration-300 ${activeSection === item.id ? 'w-8 bg-cobalt' : 'w-4 bg-onyx/20 group-hover:bg-onyx/40'}`}></div>
               <span className={`text-[10px] font-mono font-bold transition-colors ${activeSection === item.id ? 'text-cobalt' : 'text-onyx/40 group-hover:text-onyx'}`}>
                  {item.label}
               </span>
            </button>
         ))}
...
const TechSpecs: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pt-32 pb-20 bg-surface text-onyx relative">
      ...
      <div className="max-w-7xl mx-auto px-6">
         ...
         <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 relative">
             ...
             <div className="lg:col-span-9">
                 ...
                 <SectionReveal>
                    <section id="visualizer" className="scroll-mt-32 mb-24">
                       <div className="grid grid-cols-1 gap-8">
                          
                          {/* Control Panel */}
                          <div className="space-y-4 mb-6">
                             <h3 className="text-sm font-bold uppercase tracking-widest mb-4 text-onyx/40">Real-Time Diagnostics</h3>
                             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <button onClick={() => setViewMode('logic')} className={`text-left p-4 rounded-xl border transition-all duration-300 ${viewMode === 'logic' ? 'bg-cobalt border-cobalt shadow-lg shadow-cobalt/20 scale-[1.02]' : 'bg-white border-onyx/10 hover:border-cobalt/50'}`}>
                                   ...
                                   <div className={`text-xs ${viewMode === 'logic' ? 'text-white/70' : 'text-onyx/60'}`}>Active Core Load</div>
                                </button>

                                <button onClick={() => setViewMode('thermal')} className={`text-left p-4 rounded-xl border transition-all duration-300 ${viewMode === 'thermal' ? 'bg-cobalt border-cobalt shadow-lg shadow-cobalt/20 scale-[1.02]' : 'bg-white border-onyx/10 hover:border-cobalt/50'}`}>
                                   <div className="flex items-center gap-3 mb-2">
                                   ...
                                   <div className={`text-xs ${viewMode === 'thermal' ? 'text-white/70' : 'text-onyx/60'}`}>TDP Distribution</div>
                                </button>
                          ...
                          <div className="w-full bg-onyx rounded-2xl border border-onyx/5 relative overflow-hidden h-[500px] shadow-[0_0_40px_rgba(0,71,171,0.25)] flex items-center justify-center">
                 ...
                 <SectionReveal>
                    <section id="reliability" className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-24 scroll-mt-32">
                       <div>
                       ...
                       <div className="bg-white border border-onyx/10 rounded-2xl p-8 relative overflow-hidden shadow-sm h-64">
                          <div className="absolute inset-0 bg-grid-pattern bg-grid opacity-30 pointer-events-none"></div>
                          <div className="h-full relative flex items-end pb-6 pl-6 border-l border-b border-onyx/20">
                             {/* AXON CURVE (FLAT/GOOD) */}
                             <svg className="absolute inset-0 w-full h-full overflow-visible">
                                <path d="M0,180 C50,180 150,180 300,190" fill="none" stroke="#0047AB" strokeWidth="3" />
                                <text x="200" y="170" className="text-xs font-bold fill-cobalt">AXON A-1400</text>
                             </svg>
                             {/* LEGACY CURVE (STEEP/BAD) */}
                             <svg className="absolute inset-0 w-full h-full overflow-visible">
                                <path d="M0,180 C50,180 150,130 300,30" fill="none" stroke="#E5E7EB" strokeWidth="3" strokeDasharray="5 5" />
                                <text x="250" y="40" className="text-xs font-bold fill-gray-400">LEGACY 3nm</text>
                             </svg>
                          </div>
                       </div>
                    </section>
                 ...
                 {/* 3. I/O STANDARDS TABLE - RESPONSIVE */}
                 <SectionReveal>
                    <section id="protocols" className="mb-24 bg-onyx rounded-3xl p-6 md:p-12 border border-white/10 relative overflow-hidden shadow-2xl scroll-mt-32">
                       {/* Background Texture */}
                       ...
                       <div className="relative z-10">
                          ...
                          <div className="w-full mb-12 block md:hidden space-y-4">
                             {[
                                { name: "PCI Express", ver: "Gen 6.0", bw: "64 GT/s", app: "GPU Interconnect & NVMe Storage" },
                                { name: "CXL", ver: "3.0", bw: "128 GB/s", app: "Cache Coherent Memory Expansion" },
                                { name: "UCIe", ver: "1.1", bw: "32 GT/s", app: "Die-to-Die Chiplet Communication" },
                                ...
                                <div key={i} className="bg-white/5 border border-white/10 rounded-xl p-6">
                                   <div className="flex justify-between items-start mb-4">
                                      ...
                                      <span className="font-mono text-xs bg-white/10 px-2 py-1 rounded text-white/80">{row.ver}</span>
                                   </div>
                                   <div className="space-y-3">
                                      <div>
                                         <div className="text-[10px] font-mono text-white/40 uppercase mb-1">Bandwidth</div>
                                         <span className="inline-block px-2 py-1 bg-cobalt/20 text-blue-300 border border-cobalt/30 font-mono text-xs font-bold rounded shadow-[0_0_15px_rgba(0,71,171,0.2)]">
                                            {row.bw}
                                         </span>
                                      </div>
                                      <div>
                                         <div className="text-[10px] font-mono text-white/40 uppercase mb-1">Target Application</div>
                                         <p className="text-sm text-white/60 leading-tight">{row.app}</p>
                                      </div>
                                   </div>
                                </div>
                             ))}
                 ...
                 {/* 4. REFERENCE DESIGN BLUEPRINT */}
                 <SectionReveal>
                    <section id="blueprint" className="mb-32 scroll-mt-32">
                        <div className="flex items-center gap-3 mb-10">
                 ...
                 {/* 5. SPECS GRID */}
                 <SectionReveal>
                    <section id="specs" className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-12 mb-32 border-t border-onyx/10 pt-16 scroll-mt-32">
                       {SPECS.map((spec, i) => (
